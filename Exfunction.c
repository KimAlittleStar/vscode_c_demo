...
//	        EXPOTRFUNC(RST,ExternFunc_SocReset,,芯片软件复位),
	    ...
	    EXPOTRFUNC(Multi,multi,%d %d,show you %d*%d),
	    EXPOTRFUNC(Minit,motor_init,,init motor),
	    EXPOTRFUNC(MsetSpeed,motor_setSpeed,%d,set motor speed),
	    EXPOTRFUNC(MStop,motor_stop,,StopMotor),
	    EXPOTRFUNC(MShow,motor_showSpeed,,show motor Speed),
	    EXPOTRFUNC(ledON,led_ON,,turn on the lemp with power),
	    EXPOTRFUNC(ledOFF,led_OFF,,turn down the lemp),
	    EXPOTRFUNC(POWER,power,%d %d,显示%d 的%d 次方)	    
};
